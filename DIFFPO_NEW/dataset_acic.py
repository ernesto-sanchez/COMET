...
def process_func(path: str, aug_rate=1, missing_ratio=0.1, train=True, dataset_name = 'acic', current_id='0'):
    ...
    if dataset_name == 'acic2018':
        ...
        load_mask_path = os.path.join(project_path, "data_acic2018", "acic2018_mask", current_id + ".csv")
    ...
    if dataset_name == 'synthetic':
        project_path = os.path.dirname(os.path.abspath(__file__))
        load_mask_path = os.path.join(project_path, "data_acic2018", "acic2018_mask","synthetic_masked.csv")
        print(load_mask_path)
...
class tabular_dataset(Dataset):
    ...
    def __init__(
        ...
        if dataset_name == 'synthetic':
            self.eval_length = 24 # features + 5
        ...
        if dataset_name == 'acic2018':
            ...
            dataset_path = os.path.join(project_path, "data_acic2018", "acic2018_norm_data",  current_id + ".csv")

            print('dataset_path', dataset_path)

            processed_data_path = (
                #f"./data_acic2018/missing_ratio-{missing_ratio}_seed-{seed}.pk"
                os.path.join(project_path, "data_acic2018", f"missing_ratio-{missing_ratio}_seed-{seed}.pk")
            ) # modify the processed data path
            processed_data_path_norm = (
                #f"./data_acic2018/missing_ratio-{missing_ratio}_seed-{seed}_max-min_norm.pk"
                os.path.join(project_path, "data_acic2018", f"missing_ratio-{missing_ratio}_seed-{seed}_max-min_norm.pk")
        ...
        if dataset_name == 'synthetic':
            ...
            dataset_path = os.path.join(project_path, "data_acic2018","acic2018_norm_data", "synthetic_merged.csv")

            print('dataset_path', dataset_path)

            processed_data_path = (
                #f"./data_acic2018/missing_ratio-{missing_ratio}_seed-{seed}.pk"
                os.path.join(project_path, "data_acic2018", "synthetic.pk")
            ) # modify the processed data path
            processed_data_path_norm = (
                #f"./data_acic2018/missing_ratio-{missing_ratio}_seed-{seed}_max-min_norm.pk"
                os.path.join(project_path, "data_acic2018", "synthetic.pk")
...
def get_dataloader(seed=1, nfold=5, batch_size=16, missing_ratio=0.1, dataset_name = 'acic2018', current_id='0'):
    ...
    if dataset_name == 'acic2018':
        ...
        processed_data_path_norm = (
            ...
            os.path.join(project_path, "data_acic2018", f"missing_ratio-{missing_ratio}_seed-{seed}_current_id-{current_id}_max-min_norm.pk")
        )
        # if not os.path.isfile(processed_data_path_norm):
        # normalize anyway.
        print(
