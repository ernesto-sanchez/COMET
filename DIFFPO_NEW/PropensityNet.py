    ...
    def fit(self, X: torch.Tensor, y: torch.Tensor) -> "PropensityNet":
        ...
        for i in range(100):
            ...
            for b in range(n_batches):
                ...
                accuracy = torch.sum(torch.argmax(preds, dim=1) == y_next).float() / len(y_next)
            ...
            if self.early_stopping or i % self.n_iter_print == 0:
                with torch.no_grad():
                    ...
                    val_accuracy = torch.sum(torch.argmax(preds, dim=1) == y_val).float() / len(y_val)   
                    wandb.log({"val_accuracy": val_accuracy.item()})
...
def load_data(dataset_name = 'acic', current_id='0'):
    ...
    if dataset_name == 'synthetic':
        ...
        dataset_path = os.path.join(data_path, "ACIC2018", "propnet", "synthetic_propnet.csv")
